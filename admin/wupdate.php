...
<body onLoad="noBack();" onpageshow="if (event.persisted) noBack();" onUnload="">
    ...
    if (isset($_GET['updateid'])) {
        $id = $_GET['updateid'];
        $sql = "SELECT * FROM workshop WHERE wid='$id'";
        $result = mysqli_query($con, $sql);

        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                $row = mysqli_fetch_assoc($result);
                $original_wname = $row['wname'];
                $original_wdetails = $row['wdetails'];
                $original_wfees = $row['wfees'];
                $original_wstart = $row['wstart'];
                $original_wend = $row['wend'];
            } else {
        ...
        echo "Workshop ID is not provided";
    }
    ...
    if (isset($_POST['update'])) {
        ...
        $query = "UPDATE workshop SET wname='$wname', wdetails='$wdetails', wfees='$wfees', wstart='$wstart', wend='$wend', wimage='$wimage' WHERE wid='$id'";
        ...
        if ($query_run) {
            ...
            confirmButtonText: "Yes, Update Workshop",
            cancelButtonText: "No, cancel",
            reverseButtons: true
            }).then((result) => {
            if (result.isConfirmed) {
                swalWithBootstrapButtons2.fire({
                title: "Workshop Details Updated!",
                text: "Workshop updated Successfully.",
                icon: "success"
                }).then(function() {
                window.location.href = 'workshop.php'; // Redirect to desired page after updating payment status
                });
            } else if (
                ...
                swalWithBootstrapButtons2.fire({
                title: "Cancelled",
                text: "Workshop Details remains unchanged.",
                icon: "error"
                });
            }
            });
        ...
        <div class="main">
            ...
            <main class="content px-3 py-2">
                <div class="container-fluid">
                    <h3 class="ms-5">Offline Workshops</h3>
                    <div class="card ms-5 col-lg-10 col-md-10">
                        <div class="card-body">
                            <form method="POST" enctype="multipart/form-data">
                                <div class="mb-3">
                                    <label>Workshop Name</label>
                                    <input type="text" class="form-control" placeholder="Enter Workshop Name" autocomplete="off" name="wname" value="<?php echo $original_wname; ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label>Workshop Details</label>
                                    <input type="text" class="form-control" placeholder="Enter Workshop Details" autocomplete="off" name="wdetails" value="<?php echo $original_wdetails; ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label>Workshop Fees</label>
                                    <input type="text" class="form-control" placeholder="Enter Workshop Fees" autocomplete="off" name="wfees" value="<?php echo $original_wfees; ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label>Start Date</label>
                                    <input type="date" class="form-control" autocomplete="off" name="wstart" value="<?php echo $original_wstart; ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label>End Date</label>
                                    <input type="date" class="form-control" autocomplete="off" name="wend" value="<?php echo $original_wend; ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label>Workshop Image</label>
                                    <input type="file" class="form-control" autocomplete="off" name="wimage" value="<?php echo $original_wimage; ?>" required>
                                    <img src="<?php echo 'workshop/images/' . $row['wimage']; ?>" width="150px" alt="images" class="mt-2">
                                </div>
                                ...
                                <button type="submit" class="btn btn-secondary" name="submit"><a href="workshop.php" class=" text-white text-decoration-none">Cancel</a></button>
                            </form>
                        </div>
                    </div>
                </div>
